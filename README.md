...
## Identity model (no auth yet)
A person is identified by their **account id**. The acting user is passed explicitly via the
**`X-User-Id`** header — there is no authentication in step 1. A `Contact` is an address-book
entry that links to a recipient's account via `linkedAccountId`; in step 1 you transfer to an
**account id** directly (the agent tools that resolve contacts arrive in step 3).

## Seed data
| Account id | Owner | Balance (EUR) | Is a contact of user 1? |
|-----------:|-------|--------------:|-------------------------|
| 1 | Demo User | 1000.00 | — (the demo sender) |
| 2 | Alice Smith | 500.00 | Alice |
| 3 | Bob Johnson | 500.00 | Bob |
| 4 | Charlie Williams | 500.00 | Charlie |
| 5 | Daniel Anderson | 500.00 | Daniel (ambiguous) |
| 6 | Daniel Craig | 500.00 | Daniel (ambiguous) |

The demo user (account `1`) owns five contacts; two are named **Daniel** so name lookups are
ambiguous.
...
# → two contacts; note each contact's linkedAccountId to pick a recipient account
```
